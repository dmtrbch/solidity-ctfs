...
The specifice issue is in the following two lines:
...
This check will always pass if new tokens are minted to the UnstoppableVault contract, but since we (the player) hold some of the tokens, we can transfer our tokens to the UnstoppableVault contract, and make this check to alway fail, therefore no one will be able to get a flesh loan.
