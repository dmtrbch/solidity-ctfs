...
1. prepare the necessary data in our ```ClimberAttacker``` contract that will be used as a function parameters to call ```execute``` function from ```ClimberTimelock```. This data will consists the following:
  <ul>
  ...
  <li>array of bytes called dataElements, first item will hold the value of the abi encoded function signature of grantRole function (this will be used to set the ClimberAttacker to have the PROPOSER_ROLE in order for this contract to be able to schedule transactions inside ClimberTimelock)</li>
